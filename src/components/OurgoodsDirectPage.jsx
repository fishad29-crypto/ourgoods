...
const OurgoodsDirectPage = () => {
  ...
  // Ensure we get products array correctly
  const productList = getAllProducts();

  // Filter products by vendor
  const directProducts = productList.filter(p => 
    p.vendor === 'OURGOODS Direct' || 
    p.vendor === 'Ourgoods Direct' || 
    p.vendor === 'OURGOODS DIRECT'
  );
  
  // Apply category and price filters
  let filteredProducts = directProducts.filter(p => {
    let priceVal = 0;
  ...
  const categories = ['All', ...new Set(directProducts.map(p => p.category).filter(Boolean))];
    ...
    <div style={{ background: '#f5f5f5', minHeight: '100vh', paddingBottom: '80px', boxSizing: 'border-box' }}>
      ...
      <div style={{ background: '#fff', padding: '15px 20px', display: 'flex', alignItems: 'center', gap: '15px', position: 'sticky', top: 0, zIndex: 10, borderBottom: '1px solid #eaeaea' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span style={{ fontWeight: 900, fontSize: '18px', color: '#111', letterSpacing: '-0.3px' }}>OURGOODS DIRECT</span>
        </div>
      </div>
      ...
      <div style={{ padding: isMobile ? '15px' : '30px 20px', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <div>
            <h2 style={{ fontSize: isMobile ? '20px' : '24px', fontWeight: 800, margin: '0 0 4px 0' }}>Exclusive Collection</h2>
...
export default OurgoodsDirectPage;
