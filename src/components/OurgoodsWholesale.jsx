...
const OurgoodsWholesale = () => {
  ...
  const scrollLeft = () => {
    setCurrentIndex(prev => {
      ...
      return prevIdx < 0 ? Math.max(0, wholesaleProducts.length - 3) : prevIdx;
    });
  };
  const scrollRight = () => {
    setCurrentIndex(prev => {
      const next = prev + 3;
      return next >= wholesaleProducts.length ? 0 : next;
    });
  };
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                ...
                <span style={{ fontWeight: 900, fontSize: isMobile ? '14px' : '16px', color: '#fff', letterSpacing: '-0.3px', whiteSpace: 'nowrap' }}>Factory Direct</span>
              </div>
            </div>
