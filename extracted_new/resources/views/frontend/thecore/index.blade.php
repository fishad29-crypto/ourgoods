...
<style>
    @media (max-width: 767px) {
        ...
        .thecore-hero-slider .thecore-square-box {
...
{{-- Best Selling And Todays Deal section removed --}}
...
<!-- Product Filter Buttons + Products Grid -->
<div class="container mt-4 mb-3">
    <div class="d-flex align-items-center justify-content-between flex-wrap">
        <div class="d-flex align-items-center" id="product-filter-buttons">
            <button type="button" class="btn btn-sm fw-600 mr-2 mb-2 product-filter-btn" data-filter="for_you" style="border-radius: 20px; border: 1.5px solid #999; background: transparent; color: #333; padding: 6px 18px; font-size: 13px;">
                {{ translate('For You') }}
            </button>
            <button type="button" class="btn btn-sm fw-600 mr-2 mb-2 product-filter-btn active" data-filter="new_arrival" style="border-radius: 20px; border: 1.5px solid #333; background: #333; color: #fff; padding: 6px 18px; font-size: 13px;">
                {{ translate('New Arrival') }}
            </button>
            <button type="button" class="btn btn-sm fw-600 mr-2 mb-2 product-filter-btn" data-filter="best_seller" style="border-radius: 20px; border: 1.5px solid #999; background: transparent; color: #333; padding: 6px 18px; font-size: 13px;">
                {{ translate('Best Seller') }}
            </button>
            <button type="button" class="btn btn-sm fw-600 mr-2 mb-2 product-filter-btn" data-filter="combo_deals" style="border-radius: 20px; border: 1.5px solid #999; background: transparent; color: #333; padding: 6px 18px; font-size: 13px;">
                {{ translate('Combo Deals') }}
            </button>
        </div>
    </div>
</div>
...
<script>
    ...
    let page = 1;
    let activeFilter = 'new_arrival';

    // Filter button click handler
    $(document).on('click', '.product-filter-btn', function() {
        const $btn = $(this);
        ...
        $('.product-filter-btn').each(function() {
        ...
        // Show loading state
        $('#section_newest').html('<div class="text-center py-4"><i class="las la-spinner la-spin la-3x"></i></div>');
        $('#view-more-container').removeClass('d-block').addClass('d-none');

        // Fetch filtered products
        $.post('{{ route('home.section.filtered_products') }}', {
            _token: '{{ csrf_token() }}',
            filter: filter
        }, function(data) {
            ...
            $('#section_newest').html('<div class="text-center py-4 text-danger">{{ translate("Failed to load products") }}</div>');
        });
    });
