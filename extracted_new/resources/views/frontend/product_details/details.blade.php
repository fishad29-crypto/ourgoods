<div class="col-sm-12 col-lg-6">
        ...
        <div class="py-20px d-flex flex-wrap flex-xl-nowrap align-items-center">
            <div class="d-flex align-items-center bg-light p-20px rounded-2 w-100 mb-2 mb-xl-0 mr-0 mr-xl-2">
                ...
                <div>
                    ...
                    @else
                        <p class="m-0 fs-14">
                            <span class="fw-bold text-blue">{{ translate('OURGOODS Product') }}</span>
                        </p>
                    @endif
    ...
    @else
        <form id="option-choice-form" class="product-details-page">
            ...
            {{-- Combo hidden fields (populated by JS) --}}
            ...
            <div class="bg-light overflow-hidden px-20px py-20px rounded-2">
                @if ($detailedProduct->digital == 0)
                    <div class="d-flex align-items-center justify-content-between mb-md-0">
                        <div>
            <h6 class="m-0 fs-20 fw-600 text-dark" id="product-base-price">
                                @if($detailedProduct->combo_deal && $detailedProduct->comboDeals->count() > 0)
                                    @php
                                        $highestTier = $detailedProduct->comboDeals->sortByDesc('min_qty')->first();
                                        $comboPerPc = $highestTier->price / $highestTier->min_qty;
                                    @endphp
                            ...
                            @if($detailedProduct->combo_deal && $detailedProduct->comboDeals->count() > 0)
                                <div class="d-flex align-items-center gap-2 mt-1">
                                    <del class="m-0 fs-14 fw-400 text-gray">{{ home_discounted_price($detailedProduct) }}</del>
                                    <span class="fs-12 fw-600 text-success bg-soft-success px-2 py-1 rounded-1">
                                        {{ translate('for') }} {{ $highestTier->min_qty }} {{ translate('pcs combo') }}
                                    </span>
                                </div>
                            @elseif (home_price($detailedProduct) != home_discounted_price($detailedProduct))
            ...
            {{-- ===== COMBO DEAL PRICING TABLE ===== --}}
            @if ($detailedProduct->combo_deal && $detailedProduct->comboDeals->count() > 0)
            @php $comboDeals = $detailedProduct->comboDeals; @endphp
            <div class="combo-deal-box mt-4 mb-2 border border-soft-light rounded-2 overflow-hidden"
                 id="combo-deal-box"
                 data-product-id="{{ $detailedProduct->id }}"
                 data-tiers="{{ $comboDeals->map(fn($t) => ['qty'=>$t->min_qty,'price'=>$t->price])->toJson() }}"
                 data-has-colors="{{ ($detailedProduct->colors && count(json_decode($detailedProduct->colors)) > 0) ? 1 : 0 }}"
                 data-colors="{{ $detailedProduct->colors ?? '[]' }}">
                <div class="d-flex align-items-center px-20px py-10px" style="background:linear-gradient(135deg,#0080fe 0%,#0056b3 100%)">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="white" viewBox="0 0 16 16" class="mr-2">
                        <path d="M8 1a7 7 0 1 0 0 14A7 7 0 0 0 8 1zm.93 9.412-1 .25A1 1 0 1 1 6.23 8.247l.14-.035 1.014-.253a.25.25 0 1 0-.121-.484l-.14.035A1 1 0 1 1 8.77 9.764l-.14.035-.7.173a.25.25 0 1 0 .121.484l.7-.173.14-.035a1 1 0 0 1 .483 1.938l-.143.036z"/>
                    </svg>
                    <span class="fs-14 fw-bold text-white">🎁 {{ translate('Combo Deal — Save More!') }}</span>
                </div>
                <div class="px-20px py-10px">
                    <div class="d-flex flex-wrap gap-2" id="combo-tier-pills" style="gap:8px">
                        @foreach($comboDeals as $i => $tier)
                        <div class="combo-tier-pill border rounded-2 px-15px py-10px text-center cursor-pointer has-transition"
                             data-qty="{{ $tier->min_qty }}"
                             data-price="{{ $tier->price }}"
                             style="min-width:100px; cursor:pointer; {{ $i==0 ? 'border-color:#0080fe;background:#f0f8ff;' : 'border-color:#dee2e6;' }}"
                             onclick="selectComboPill(this, {{ $tier->min_qty }}, {{ $tier->price }})">
                            <div class="fs-16 fw-bold text-dark">{{ get_system_default_currency()->symbol }}{{ number_format($tier->price, 0) }}</div>
                            <div class="fs-12 text-gray">{{ $tier->min_qty }} {{ translate('pcs') }}</div>
                            @php $per = round($tier->price / $tier->min_qty, 0); @endphp
                            <div class="fs-11 text-blue mt-1">≈ {{ get_system_default_currency()->symbol }}{{ number_format($per, 0) }}/{{ translate('pc') }}</div>
                        </div>
                        @endforeach
                    </div>
                </div>
                ...
                @if ($detailedProduct->variant_product)
                    ...
                    <div class="px-20px py-10px border border-soft-light rounded-2 product-variant position-relative collapsed">
                        ...
                        @if ($detailedProduct->colors != null && count(json_decode($detailedProduct->colors)) > 0)
                            <div class="py-10px {{ $product_variations>1 ? 'variant-item':''}}" id="color-section">
                                <div class="d-flex align-items-center justify-content-between mb-3">
                                    <p class="mb-0 fs-14 fw-bold text-dark">{{ translate('Color') }}</p>
                                    @if($detailedProduct->combo_deal && $detailedProduct->comboDeals->count() > 0)
                                    <span class="fs-12 text-blue fw-400" id="combo-color-hint" style="display:none">
                                        {{ translate('Select') }} <span id="combo-color-limit">1</span> {{ translate('color(s)') }}
                                        — <span id="combo-color-selected-count">0</span> {{ translate('selected') }}
                                    </span>
                                    @endif
                                </div>

                                @if($detailedProduct->combo_deal && $detailedProduct->comboDeals->count() > 0)
                                {{-- COMBO COLOR MULTI-SELECT with per-color quantity --}}
                                <div class="variant-wrapper" id="combo-color-wrapper">
                                    @foreach (json_decode($detailedProduct->colors) as $key => $color)
                                    @php $colorName = get_single_color_name($color); @endphp
                                    <div class="aiz-megabox rounded-1 bg-white mb-1 d-inline-block position-relative"
                                         data-color="{{ $colorName }}"
                                         data-hex="{{ $color }}"
                                         style="vertical-align:top;">
                                        <input type="checkbox" class="combo-color-checkbox" value="{{ $colorName }}">
                                        <div class="d-flex align-items-center aiz-megabox-elem px-10px cursor-pointer"
                                             onclick="comboColorToggle(this.parentElement)">
                                            <span class="w-15px h-15px rounded-circle flex-shrink-0" style="background-color:{{ $color }};"></span>
                                            <span class="fs-14 fw-400 text-dark pl-2 pr-1">{{ $colorName }}</span>
                                        </div>
                                        {{-- Qty control: shown when color is selected --}}
                                        <div class="combo-color-qty-wrap d-none" style="padding:4px 8px 6px;">
                                            <div class="d-flex align-items-center justify-content-center" style="gap:0;">
                                                <button type="button" class="btn btn-sm p-0 border-0 lh-1"
                                                        style="width:22px;height:22px;font-size:16px;font-weight:700;background:#f0f0f0;border-radius:4px;color:#333;"
                                                        onclick="comboColorQty(this.closest('.aiz-megabox'), -1)">−</button>
                                                <span class="combo-color-qty-val fw-700 fs-14 text-center" style="min-width:24px;">1</span>
                                                <button type="button" class="btn btn-sm p-0 border-0 lh-1"
                                                        style="width:22px;height:22px;font-size:16px;font-weight:700;background:#f0f0f0;border-radius:4px;color:#333;"
                                                        onclick="comboColorQty(this.closest('.aiz-megabox'), 1)">+</button>
                                            </div>
                                ...
                                {{-- NORMAL COLOR (hidden in combo mode, shown in normal mode) --}}
                                <div class="variant-wrapper d-none" id="normal-color-wrapper">
                                    @foreach (json_decode($detailedProduct->colors) as $key => $color)
                                        <label class="aiz-megabox rounded-1 bg-white cursor-pointer" data-title="{{ get_single_color_name($color) }}" data-color-hex="{{ $color }}">
            ...
            @else
                <div class="border-dashed border-1 border-soft-light rounded-2 overflow-hidden mt-4 mb-1 px-20px pt-15px pb-20px">
                    ...
                    @endif
                    <!--Buttons Start-->
                    @if ($detailedProduct->stock_out == 1)
                        {{-- Manual Stock Out — hide cart buttons --}}
                        <div class="d-flex flex-wrap flex-md-nowrap align-items-center">
                            <button type="button" class="fw-600 text-white border-0 rounded-1 fs-14 fw-bold has-transition py-20px px-20px w-100" style="background-color:#dc3545; cursor:not-allowed;" disabled>
                                <i class="la la-ban"></i> {{ translate('Stock Out') }}
                            </button>
                        </div>
                    @else
                        <div class="d-flex flex-wrap flex-md-nowrap align-items-center">
                    ...
                    {{-- COMBO status badge (no button needed — regular Add to Cart handles it) --}}
                    @if($detailedProduct->combo_deal && $detailedProduct->comboDeals->count() > 0)
                    <div class="mt-2" id="combo-status-badge" style="display:none">
                        <div class="d-flex align-items-center px-10px py-5px rounded-1" style="background:#e8f4ff;border:1px solid #b3d7ff">
                            <span style="font-size:14px">🎁</span>
                            <span class="fs-12 fw-600 text-blue ml-1" id="combo-status-text">{{ translate('Combo Deal Applied') }}</span>
                        </div>
                    </div>
                    @endif
            ...
            <div class="py-20px d-flex flex-wrap flex-xl-nowrap align-items-center">
                <div class="d-flex align-items-center bg-light p-20px rounded-2 w-100 mb-2 mb-xl-0 mr-0 mr-xl-2">
                    ...
                    <div>
                        ...
                        @else
                            <p class="m-0 fs-14">
                                <span class="fw-bold text-dark">{{ translate('OURGOODS Product') }}</span>
                            </p>
                        @endif
                        @if (get_setting('conversation_system') == 1)
                            <a href="javascript:void();" onclick="show_chat_modal()" class="fs-14 fw-400 text-blue animate-underline-blue has-transition">{{ translate('Message Seller') }}</a>
