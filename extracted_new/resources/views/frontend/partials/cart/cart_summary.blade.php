    ...
    <div class="card rounded-0 border">
        ...
        @foreach ($carts as $key => $cartItem)
            @php
                ...
                $unitPriceRaw = cart_product_price($cartItem, $cartItem->product, false, false);
                $unitPriceRaw2 = cart_product_price($cartItem, $product, false, false);
                if ($isComboCart) {
                    $subtotal_for_min_order_amount += round($unitPriceRaw * $cartItem['quantity']);
                    $subtotal += round($unitPriceRaw2 * $cartItem['quantity']);
                } else {
                    $subtotal_for_min_order_amount += $unitPriceRaw * $cartItem['quantity'];
                    $subtotal += $unitPriceRaw2 * $cartItem['quantity'];
                }
