                            ...
                            @if (!empty($admin_products))
                                    ...
                                    <div class="aiz-checkbox-inline">
                                        <label class="aiz-checkbox d-block">
                                            ...
                                            <span class="fs-16 fw-700 text-dark ml-3 pb-3 d-block border-left-0 border-top-0 border-right-0 border-bottom border-dashed">
                                                {{ translate('OURGOODS Products') }} ({{ count($admin_products) }})
                                            </span>
                                            <span class="aiz-square-check"></span>
                                        </label>
                                    </div>
                                ...
                                @foreach ($admin_products as $key => $product_id)
                                    @php
                                        ...
                                        $stockVar = $cartItem->variation; if(preg_match('/^combo_\d+_(.+)$/', $stockVar, $_m)) $stockVar = $_m[1];
                                        $product_stock = $product->stocks->where('variant', $stockVar)->first() ?? $product->stocks->first();
                                        $isComboCart = preg_match('/^combo_\d+/', $cartItem->variation ?? '');
                                        $unitPrice = cart_product_price($cartItem, $product, false);
                                        $lineTotal = $isComboCart ? round($unitPrice * $cartItem->quantity) : $unitPrice * $cartItem->quantity;
                                        ...
                                        <div class="row gutters-5 align-items-center">
                                            ...
                                            <div class="col-md-5 col-10 d-flex align-items-center mb-2 mb-md-0">
                                                ...
                                                <span>
                                                    ...
                                                    @if ($admin_product_variation[$key] != '' && !str_starts_with($admin_product_variation[$key], 'combo_'))
                                                        <span class="fs-12 text-secondary">{{ translate('Variation') }}: {{ $admin_product_variation[$key] }}</span>
                                                    @endif
                                                    @if ($cartItem->combo_colors)
                                                        @php $comboColorsArr = is_array(json_decode($cartItem->combo_colors, true)) ? json_decode($cartItem->combo_colors, true) : []; @endphp
                                                        @if(count($comboColorsArr) > 0)
                                                        @php $cartColorGrouped = array_count_values($comboColorsArr); @endphp
                                                        <div class="d-flex align-items-center flex-wrap mt-2" style="gap:6px">
                                                            <span class="fs-12 fw-600" style="color:#555">🎁</span>
                                                            @foreach($cartColorGrouped as $ccName => $ccCount)
                                                            <span class="d-inline-flex align-items-center border rounded-1 px-1" style="gap:3px;border-color:#dee2e6!important;line-height:1.5;">
                                                                <span style="display:inline-block;width:12px;height:12px;border-radius:50%;background:{{ get_single_color_code($ccName) }};border:1px solid #ccc;"></span>
                                                                <small class="text-dark fw-600">{{ $ccName }}@if($ccCount > 1) ×{{ $ccCount }}@endif</small>
                                                            </span>
                                                            @endforeach
                                                        </div>
                                                        @endif
                                            ...
                                            <div class="col-md col-4 ml-4 ml-sm-0 my-3 my-md-0 d-flex flex-column ml-sm-5 ml-md-0">
                                                ...
                                                <span class="fw-700 fs-14 mb-2">{{ $isComboCart ? single_price(round($unitPrice)) : cart_product_price($cartItem, $product, true, false) }}</span>
                                            ...
                                            <div class="col-xl-4 col-md-3 col d-flex flex-column flex-xl-row justify-content-xl-between align-items-xl-center">
                                                ...
                                                <div class="mr-2 mt-2 mt-xl-0">
                                                    <span class="fw-700 fs-14 text-primary">{{ single_price($lineTotal) }}</span>
                                                </div>
                                            </div>
                                            <!-- Remove From Cart -->
                                ...
                                @foreach ($seller_products as $key => $seller_product)
                                    ...
                                    @foreach ($seller_product as $key2 => $product_id)
                                        @php
                                            ...
                                            $stockVar2 = $cartItem->variation; if(preg_match('/^combo_\d+_(.+)$/', $stockVar2, $_m2)) $stockVar2 = $_m2[1];
                                            $product_stock = $product->stocks->where('variant', $stockVar2)->first() ?? $product->stocks->first();
                                            $isComboCart = preg_match('/^combo_\d+/', $cartItem->variation ?? '');
                                            $unitPrice = cart_product_price($cartItem, $product, false);
                                            $lineTotal = $isComboCart ? round($unitPrice * $cartItem->quantity) : $unitPrice * $cartItem->quantity;
                                            $total = $total + $lineTotal;
                                        ...
                                        <li class="list-group-item px-0 border-md-0">
                                            <div class="row gutters-5 align-items-center">
                                                ...
                                                <div class="col-md-5 col-10 d-flex align-items-center mb-2 mb-md-0">
                                                    ...
                                                    <span>
                                                        <span class="fs-14 fw-400 text-dark text-truncate-2 mb-2">{{ $product->getTranslation('name') }}</span>
                                                        @if ($seller_product_variation[$key][$key2] != '' && !str_starts_with($seller_product_variation[$key][$key2], 'combo_'))
                                                            <span class="fs-12 text-secondary">{{ translate('Variation') }}: {{ $seller_product_variation[$key][$key2] }}</span>
                                                        @endif
                                                        @if ($cartItem->combo_colors)
                                                            @php $comboColorsArr2 = is_array(json_decode($cartItem->combo_colors, true)) ? json_decode($cartItem->combo_colors, true) : []; @endphp
                                                            @if(count($comboColorsArr2) > 0)
                                                            @php $cartColorGrouped2 = array_count_values($comboColorsArr2); @endphp
                                                            <div class="d-flex align-items-center flex-wrap mt-2" style="gap:6px">
                                                                <span class="fs-12 fw-600" style="color:#555">🎁</span>
                                                                @foreach($cartColorGrouped2 as $ccName2 => $ccCount2)
                                                                <span class="d-inline-flex align-items-center border rounded-1 px-1" style="gap:3px;border-color:#dee2e6!important;line-height:1.5;">
                                                                    <span style="display:inline-block;width:12px;height:12px;border-radius:50%;background:{{ get_single_color_code($ccName2) }};border:1px solid #ccc;"></span>
                                                                    <small class="text-dark fw-600">{{ $ccName2 }}@if($ccCount2 > 1) ×{{ $ccCount2 }}@endif</small>
                                                                </span>
                                                                @endforeach
                                                            </div>
                                                            @endif
