@php
    ...
    if (count($carts) > 0) {
        foreach ($carts as $key => $cartItem) {
            ...
            $rawLine = cart_product_price($cartItem, $product, false) * $cartItem['quantity'];
            $total = $total + (preg_match('/^combo_\d+/', $cartItem['variation'] ?? '') ? round($rawLine) : $rawLine);
        }
    }
@endphp
                        ...
                        <span class="d-flex align-items-center">
                            <a href="{{ route('product', $product->slug) }}"
                                ...
                                <span class="minw-0 pl-2 flex-grow-1">
                                    ...
                                    <span class="fs-14 fw-400 text-secondary">{{ preg_match('/^combo_\d+/', $cartItem['variation'] ?? '') ? single_price(round(cart_product_price($cartItem, $product, false))) : cart_product_price($cartItem, $product) }}</span>
                                </span>
                            </a>
                            <span class="">
