@php
    ...
    if ($product->digital == 0) {
        ...
        $totalStock = 999; // digital products always in stock
    }
    $isOutOfStock = $product->stock_out == 1 || $totalStock <= 0;
@endphp
<div class="aiz-card-box h-auto bg-white py-3 hov-scale-img {{ $isOutOfStock ? 'is-stock-out' : '' }}">
    <div class="position-relative  img-fit overflow-hidden" >
        ...
        @endif
        <!-- Combo Deal tag -->
        @if ($product->combo_deal && $product->comboDeals->count() > 0)
            <span class="absolute-top-left rounded rounded-4 fs-11 text-white fw-700 px-2 lh-1-8 ml-1 mt-1"
                style="background-color:#0080fe; top:{{ 25 * $badgeIndex }}px;">
                {{ translate('Combo') }}
            </span>
            @php $badgeIndex++; @endphp
        @endif
        ...
        @if ($isOutOfStock)
            <span class="absolute-top-left rounded rounded-4 fs-11 text-white fw-700 px-2 lh-1-8 ml-1 mt-1"
                style="background-color:#dc3545; top:{{ 25 * $badgeIndex }}px;">
                {{ translate('Stock Out') }}
            </span>
            @php $badgeIndex++; @endphp
        @endif
        ...
        @if ($product->auction_product == 0)
            ...
            @if ($isOutOfStock)
                {{-- Stock Out - show disabled button --}}
                <div class="cart-btn absolute-bottom-left w-100 h-35px aiz-p-hov-icon d-none d-sm-flex flex-column justify-content-center align-items-center active"
                    style="background-color:#dc3545 !important; cursor:not-allowed; opacity:0.9;">
                    <span class="text-white fs-13 fw-700">{{ translate('Stock Out') }}</span>
                </div>
            @elseif ( (is_array($colors) && count($colors) > 0) || (is_array($attributes) && count($attributes) > 0) )
