								...
								@if ($orderDetail->product != null)
									<tr class="">
										...
										@if ($orderDetail->combo_colors)
											@php $invColors = is_array(json_decode($orderDetail->combo_colors, true)) ? json_decode($orderDetail->combo_colors, true) : []; @endphp
											@if(count($invColors) > 0)
												<br>
												@php $invGrouped = array_count_values($invColors); @endphp
								<small style="color:#0080fe;">&#9733; Combo ({{ count($invColors) }} pcs): @foreach($invGrouped as $cName => $cCount){{ $cName }}@if($cCount > 1) ×{{ $cCount }}@endif{{ $loop->last ? '' : ', ' }}@endforeach</small>
											@endif
										@endif
										</td>
