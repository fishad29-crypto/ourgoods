                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($orderDetail->product != null && $orderDetail->product->auction_product == 0)
                                            ...
                                            @if ($orderDetail->combo_colors)
                                                @php $adminComboColors = is_array(json_decode($orderDetail->combo_colors, true)) ? json_decode($orderDetail->combo_colors, true) : []; @endphp
                                                @if(count($adminComboColors) > 0)
                                                <div class="d-flex align-items-center flex-wrap mt-1" style="gap:4px">
                                                    <small class="fw-600 text-info">🎁 Combo ({{ count($adminComboColors) }} pcs):</small>
                                                    @foreach($adminComboColors as $acc)
                                                    <span title="{{ $acc }}" style="display:inline-block;width:16px;height:16px;border-radius:50%;background:{{ $acc }};border:2px solid #ddd;box-shadow:0 1px 2px rgba(0,0,0,0.15)"></span>
                                                    @endforeach
                                                </div>
                                                @endif
                                            @endif
