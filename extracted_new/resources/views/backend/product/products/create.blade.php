...
@section('content')
    ...
    <div class="page-content">
        ...
        <div class="px-3 px-md-2rem add-product-page-content mb-4">
            ...
            <form action="{{route('products.store')}}" method="POST" enctype="multipart/form-data" id="aizSubmitForm" autocomplete="off">
                @csrf
                <div class="row">
                    <div class="col-xl-8">
                        ...
                        <div class="border border-gray-300 rounded-2 px-3 px-lg-4 py-3 py-lg-4 mt-4">
                            ...
                            <div class="form-group row gutters-5">
                                ...
                                <div class="col-md-8">
                                    ...
                                    <select class="form-control aiz-selectpicker" data-live-search="true" data-selected-text-format="count" name="colors[]" id="colors" multiple disabled autocomplete="off">
                                        @foreach ($colors as $key => $color)
                                        <option  value="{{ $color->code }}" data-name="{{ $color->name }}" data-content="<span><span class='size-15px d-inline-block mr-2 rounded border' style='background:{{ $color->code }}'></span><span>{{ $color->name }}</span></span>"></option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="form-group row gutters-5" id="color-images-section" style="display:none;">
                                <div class="col-md-3">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group mb-2 mb-lg-3">
                                        <label for="unit-price"
                                            ...
                                            <span class="text-danger">*</span></label>
                                        <input type="number" lang="en" min="0" value="0" step="0.01" placeholder="{{ translate('Selling price') }}" name="unit_price" class="form-control @error('unit_price') is-invalid @enderror" onkeyup="calculateProfit()">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group mb-2 mb-lg-3">
                                        ...
                                        <input type="number" lang="en" min="0" value="0" step="0.01" placeholder="{{ translate('Buying price') }}" name="purchase_price" class="form-control" onkeyup="calculateProfit()">
                                    </div>
                                </div>
                    ...
                    <div class="col-xl-4">
                        ...
                        <div class="border border-gray-300 rounded-2 px-3 px-lg-4 py-3 py-lg-4 mt-4">
                            ...
                            <div class="d-flex align-items-center mb-3">
                                <label class="aiz-switch aiz-switch-blue mb-0 pr-2">
                                    <input value="1" type="checkbox" name="combo_deal" id="combo_deal_toggle" onchange="toggleComboDeal(this)">
                                    <span></span>
                                </label>
                                <span class="fs-14 fw-400 d-block" style="margin-top:-6px">{{ translate('Enable Combo Deal') }}</span>
                            </div>
                            <div id="combo_deal_section" style="display:none">
                                ...
                                <div class="form-group mb-0 warranty_selection_div d-none">
                                    <select class="form-control aiz-selectpicker mt-2" name="warranty_id" id="warranty_id">
                                        ...
                                        @foreach ($warranties as $warranty)
                                            <option value="{{ $warranty->id }}" @selected(old('warranty_id') == $warranty->id)>{{ $warranty->text }}</option>
                                        @endforeach
                                    </select>
                                </div>
...
<script type="text/javascript">
    ...
    // Reset colors on create page load (after bootstrap-select is ready)
    $(window).on('load', function() {
        setTimeout(function() {
            $('input[name="colors_active"]').prop('checked', false);
            $('#colors').val([]).prop('disabled', true);
            AIZ.plugins.bootstrapSelect('refresh');
            $('#selected-colors-tags').empty();
        }, 300);
    });
    ...
    // Build color-image assignment inputs
    function renderColorImageInputs() {
        ...
        selectedColors.forEach(function(c) {
            ...
            var html = '<div class="mb-3 p-3 border rounded" style="background:#fafbfc;">' +
                ...
                '<input type="hidden" name="color_images[' + c.code + ']" class="selected-files" value="">' +
                '</div>' +
                '<div class="file-preview box sm mt-2"></div>' +
                '</div>';
            container.append(html);
        });
        // Re-initialize AIZ uploader for new elements
...
function removeComboTier(btn) {
    ...
    if (rows.length > 1) {
        btn.closest('.combo-tier-row').remove();
    }
}
</script>
