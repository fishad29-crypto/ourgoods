        ...
        <div class="tab-filter-bar">
            <form class="" id="sort_products" action="" method="GET">
                <div class="card-header row  border-0 pb-0 mt-2">
                    ...
                    <!--Category Filter-->
                    <div class="col-md-2 mb-1 mb-md-0 px-0 px-md-1 inner-select">
                        <select class="form-control aiz-selectpicker mb-2 mb-md-0 bg-light" id="category_filter" name="category_id" data-live-search="true" onchange="filterByCategory(this)">
                            <option value="" class="hov-bg-light text-secondary fs-14 fw-400">{{ translate('All Categories') }}</option>
                            @php
                                $allCats = \App\Models\Category::where('digital', 0)
                                    ->select('id', 'name', 'parent_id')
                                    ->get();
                                function buildCatOptions($cats, $parentId = 0, $depth = 0) {
                                    $result = collect();
                                    $children = $cats->where('parent_id', $parentId)->sortBy('name', SORT_NATURAL | SORT_FLAG_CASE);
                                    foreach ($children as $child) {
                                        $child->display_name = str_repeat('— ', $depth) . $child->name;
                                        $result->push($child);
                                        $result = $result->merge(buildCatOptions($cats, $child->id, $depth + 1));
                                    }
                                    return $result;
                                }
                                $sortedCats = buildCatOptions($allCats);
                            @endphp
                            @foreach ($sortedCats as $cat)
                                <option value="{{ $cat->id }}" class="hov-bg-light text-secondary fs-14 fw-400">
                                    {{ $cat->display_name }}
                                </option>
                            @endforeach
                        </select>
...
@section('script')
    <script type="text/javascript">
        ...
        function update_stock_out(el){
            ...
            $.post('{{ route('products.stock_out') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Stock Out status updated successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
