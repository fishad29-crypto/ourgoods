            ...
            <form action="{{route('products.update', $product->id)}}" method="POST" enctype="multipart/form-data" id="aizSubmitForm">
                ...
                <div class="row">
                    <div class="col-xl-8">
                                        ...
                                        <div class="form-group mb-2 mb-lg-3" id="category">
                                            ...
                                            <select class="form-control aiz-selectpicker" name="category_id" id="category_id"
                                                ...
                                                @foreach ($categories as $category)
                                                    <option value="{{ $category->id }}" @if($product->category_id == $category->id) selected @endif>{{ $category->display_name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                        ...
                        <div class="border border-gray-300 rounded-2 px-3 px-lg-4 py-3 py-lg-4 mt-4">
                            ...
                            <div class="form-group row gutters-5">
                                ...
                                <div class="col-md-8">
                                    <div id="selected-colors-tags" class="mb-2" style="display:flex;flex-wrap:wrap;gap:5px;"></div>
                                    <select class="form-control aiz-selectpicker" data-live-search="true" data-selected-text-format="count" name="colors[]" id="colors" multiple @if(count(json_decode($product->colors)) < 1) disabled @endif>
                                        ...
                                        <option value="{{ $color->code }}" data-name="{{ $color->name }}" data-content="<span><span class='size-15px d-inline-block mr-2 rounded border' style='background:{{ $color->code }}'></span><span>{{ $color->name }}</span></span>"
                            ...
                            @php
                                $colorImagesData = json_decode($product->color_images ?? '{}', true) ?? [];
                                $selectedColors = json_decode($product->colors, true) ?? [];
                            @endphp
                            <div class="form-group row gutters-5" id="color-images-section" style="{{ count($selectedColors) > 0 ? '' : 'display:none;' }}">
                                <div class="col-md-3">
                                    <label class="form-label fs-13 fw-600">{{ translate('Assign Images to Colors') }}</label>
                                </div>
                                <div class="col-md-9" id="color-images-container">
                                    @foreach ($selectedColors as $colorCode)
                                        @php
                                            $colorName = \App\Models\Color::where('code', $colorCode)->first()->name ?? $colorCode;
                                            $colorImageIds = $colorImagesData[$colorCode] ?? '';
                                        @endphp
                                        <div class="mb-3 p-3 border rounded" style="background:#fafbfc;">
                                            <div class="d-flex align-items-center mb-2">
                                                <span style="width:18px;height:18px;border-radius:50%;background:{{ $colorCode }};border:1px solid #ccc;display:inline-block;margin-right:8px;"></span>
                                                <span class="fw-600 fs-13">{{ $colorName }}</span>
                                            </div>
                                            <div class="input-group" data-type="image" data-multiple="true" data-toggle="aizuploader">
                                                <div class="input-group-prepend">
                                                    <div class="input-group-text bg-soft-secondary font-weight-medium">{{ translate('Browse') }}</div>
                                                </div>
                                                <div class="form-control file-amount">{{ $colorImageIds ? count(explode(',', $colorImageIds)) . ' ' . translate('file(s) chosen') : translate('Choose Files') }}</div>
                                                <input type="hidden" name="color_images[{{ $colorCode }}]" class="selected-files" value="{{ $colorImageIds }}">
                                            </div>
                                            <div class="file-preview box sm mt-2">
                                                @if($colorImageIds)
                                                    @foreach(explode(',', $colorImageIds) as $imgId)
                                                        @if(trim($imgId))
                                                        <div class="d-flex justify-content-between align-items-center mt-2 file-preview-item" data-id="{{ trim($imgId) }}">
                                                            <div class="align-items-center d-flex">
                                                                <img src="{{ uploaded_asset(trim($imgId)) }}" class="img-fit size-60px" style="border-radius:4px;">
                                                            </div>
                                                            <div class="file-preview-actions">
                                                                <a href="javascript:void(0)" class="btn btn-sm btn-icon btn-circle btn-soft-danger" data-toggle="remove-parent" data-parent=".file-preview-item">
                                                                    <i class="las la-times"></i>
                                                                </a>
                                                            </div>
                                                        </div>
                            ...
                            <div class="customer_choice_options mb-4" id="customer_choice_options">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-9">
                                        <div class="selected-attr-tags mb-2" style="display:flex;flex-wrap:wrap;gap:5px;"></div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group mb-2 mb-lg-3">
                                        <label for="unit-price"
                                            class="col-from-label fs-14 fw-500">{{ translate('Selling Price') }}
                                            <span class="text-danger">*</span></label>
                                        <input type="number" lang="en" min="0" value="{{ $product->unit_price }}" step="0.01" placeholder="{{ translate('Selling price') }}" name="unit_price" class="form-control @error('unit_price') is-invalid @enderror" onkeyup="calculateProfit()">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group mb-2 mb-lg-3">
                                        <label for="purchase-price"
                                            class="col-from-label fs-14 fw-500">{{ translate('Buying Price') }}
                                            <small class="text-muted">({{ translate('Cost of Goods') }})</small></label>
                                        <input type="number" lang="en" min="0" value="{{ $product->purchase_price ?? 0 }}" step="0.01" placeholder="{{ translate('Buying price') }}" name="purchase_price" class="form-control" onkeyup="calculateProfit()">
                                    </div>
                                </div>
                                <div class="col-12" id="profit-display" style="display:none;">
                                    <div class="alert mb-2 py-2 px-3 d-flex align-items-center justify-content-between" id="profit-alert" style="border-radius:6px;">
                                        <span class="fs-13 fw-600"><i class="las la-chart-line mr-1"></i>{{ translate('Estimated Profit') }}:</span>
                                        <span class="fs-16 fw-700" id="profit-value"></span>
                                    </div>
                                </div>
                    ...
                    <div class="col-xl-4">
                        ...
                        <!-- Combo Deal Start -->
                        @php $existingTiers = $product->comboDeals; @endphp
                        <div class="border border-gray-300 rounded-2 px-3 px-lg-4 py-3 py-lg-4 mt-4">
                            <h5 class="fs-16 fw-700 border-bottom-dashed mb-3 pb-2">{{ translate('Combo Deal') }}</h5>
                            <div class="d-flex align-items-center mb-3">
                                <label class="aiz-switch aiz-switch-blue mb-0 pr-2">
                                    <input value="1" type="checkbox" name="combo_deal" id="combo_deal_toggle" onchange="toggleComboDeal(this)" @if($product->combo_deal) checked @endif>
                                    <span></span>
                                </label>
                                <span class="fs-14 fw-400 d-block" style="margin-top:-6px">{{ translate('Enable Combo Deal') }}</span>
                            </div>
                            <div id="combo_deal_section" @if(!$product->combo_deal) style="display:none" @endif>
                                <p class="fs-12 text-muted mb-2">{{ translate('Set tiered prices. Customer gets combo price when quantity matches.') }}</p>
                                <div id="combo_tiers">
                                    @if($existingTiers->count() > 0)
                                        @foreach($existingTiers as $tier)
                                        <div class="combo-tier-row d-flex align-items-center mb-2">
                                            <input type="number" name="combo_qty[]" placeholder="Min Qty" min="2" value="{{ $tier->min_qty }}" class="form-control mr-2" style="width:100px" @if($product->combo_deal) required @endif>
                                            <input type="number" name="combo_price[]" placeholder="Price (৳)" min="0" step="0.01" value="{{ $tier->price }}" class="form-control mr-2" @if($product->combo_deal) required @endif>
                                            <button type="button" class="btn btn-icon btn-soft-danger btn-circle btn-sm" onclick="removeComboTier(this)">
                                                <i class="las la-times"></i>
                                            </button>
                                        </div>
                                        @endforeach
                                    @else
                                        <div class="combo-tier-row d-flex align-items-center mb-2">
                                            <input type="number" name="combo_qty[]" placeholder="Min Qty" min="2" class="form-control mr-2" style="width:100px">
                                            <input type="number" name="combo_price[]" placeholder="Price (৳)" min="0" step="0.01" class="form-control mr-2">
                                            <button type="button" class="btn btn-icon btn-soft-danger btn-circle btn-sm" onclick="removeComboTier(this)">
                                                <i class="las la-times"></i>
                                            </button>
                                        </div>
                                ...
                                <button type="button" class="btn btn-block border border-dashed border-gray-400 hov-bg-soft-secondary fs-14 rounded-2 d-flex align-items-center justify-content-center mt-2" onclick="addComboTier()">
                                    <i class="las la-plus mr-2"></i> {{ translate('Add Tier') }}
                                </button>
                            </div>
                        </div>
                        <!-- Combo Deal End -->
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-md-10">
                                                    ...
                                                    >
                                                        ...
                                                        @foreach ($categories as $category)
                                                            <option value="{{ $category->id }}" @if($fqCategory == $category->id) selected @endif>{{ $category->display_name }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
...
<script type="text/javascript">
    ...
    // Build color-image assignment inputs (for newly added colors)
    function renderColorImageInputs() {
        var container = $('#color-images-container');
        var section = $('#color-images-section');
        // Preserve existing values
        var existingValues = {};
        container.find('input[name^="color_images"]').each(function() {
        ...
        $('#colors option:selected').each(function() {
            ...
            if(code && name) {
                selectedColors.push({code: code, name: name});
            }
        });
        ...
        selectedColors.forEach(function(c) {
            var inputName = 'color_images[' + c.code + ']';
            var existingVal = existingValues[inputName] || '';
            var fileText = existingVal ? '{{ translate("Files selected") }}' : '{{ translate("Choose Files") }}';
            var html = '<div class="mb-3 p-3 border rounded" style="background:#fafbfc;">' +
                '<div class="d-flex align-items-center mb-2">' +
                '<span style="width:18px;height:18px;border-radius:50%;background:' + c.code + ';border:1px solid #ccc;display:inline-block;margin-right:8px;"></span>' +
                '<span class="fw-600 fs-13">' + c.name + '</span>' +
                '</div>' +
                '<div class="input-group" data-type="image" data-multiple="true" data-toggle="aizuploader">' +
                '<div class="input-group-prepend">' +
                '<div class="input-group-text bg-soft-secondary font-weight-medium">{{ translate("Browse") }}</div>' +
                '</div>' +
                '<div class="form-control file-amount">' + fileText + '</div>' +
                '<input type="hidden" name="' + inputName + '" class="selected-files" value="' + existingVal + '">' +
                '</div>' +
                '<div class="file-preview box sm mt-2"></div>' +
                '</div>';
            container.append(html);
        });
        AIZ.uploader.initForAll();
    }

    // Render selected color tags
    function renderColorTags() {
        var container = $('#selected-colors-tags');
        container.empty();
        $('#colors option:selected').each(function() {
            var code = $(this).val();
            var name = $(this).data('name') || $(this).text().trim();
            if(code && name) {
                container.append(
                    '<span style="display:inline-flex;align-items:center;gap:4px;padding:3px 10px;border-radius:20px;background:#f1f3f5;border:1px solid #dee2e6;font-size:12px;">' +
                    '<span style="width:14px;height:14px;border-radius:50%;background:' + code + ';border:1px solid #ccc;display:inline-block;"></span>' +
                    '<span>' + name + '</span></span>'
                );
            }
        });
    ...
    // Render selected attribute value tags
    function renderAttrTags(selectEl) {
        var $select = $(selectEl);
        // Go up to .form-group level to find tags container (bootstrap-select wraps select in a div)
        var $container = $select.closest('.form-group').find('.selected-attr-tags');
        ...
        $select.find('option:selected').each(function() {
            ...
            if(val) {
                $container.append(
                    '<span style="display:inline-flex;align-items:center;padding:3px 10px;border-radius:20px;background:#e7f5ff;border:1px solid #74c0fc;font-size:12px;color:#1971c2;">' + val + '</span>'
                );
            }
        });
    }

    // Listen for attribute choice changes
    $(document).on('change', '.attribute_choice', function() {
        renderAttrTags(this);
    });

    // Render tags on page load (after everything is fully loaded)
    $(window).on('load', function() {
        setTimeout(function() {
            renderColorTags();
            $('select.attribute_choice').each(function() {
                renderAttrTags(this);
            });
        }, 200);
    });
    ...
    function calculateProfit() {
        var selling = parseFloat($('input[name="unit_price"]').val()) || 0;
        var buying = parseFloat($('input[name="purchase_price"]').val()) || 0;
        var profitDiv = $('#profit-display');
        var profitVal = $('#profit-value');
        var profitAlert = $('#profit-alert');
        if (selling > 0 || buying > 0) {
            var profit = selling - buying;
            var pct = buying > 0 ? ((profit / buying) * 100).toFixed(1) : '∞';
            profitDiv.show();
            if (profit >= 0) {
                profitAlert.css({'background-color': '#d4edda', 'border': '1px solid #28a745', 'color': '#155724'});
                profitVal.html('৳' + profit.toFixed(2) + ' <small class="fw-400">(' + pct + '% margin)</small>');
            } else {
                profitAlert.css({'background-color': '#f8d7da', 'border': '1px solid #dc3545', 'color': '#721c24'});
                profitVal.html('-৳' + Math.abs(profit).toFixed(2) + ' <small class="fw-400">(Loss!)</small>');
            }
        } else {
            profitDiv.hide();
        }
...
<!-- Removed temp data script include -->
 ...
 // AI Generation Function
    ...
    function toggleComboDeal(checkbox) {
        var section = document.getElementById('combo_deal_section');
        section.style.display = checkbox.checked ? 'block' : 'none';
        // Toggle required on combo inputs
        var inputs = section.querySelectorAll('input[name="combo_qty[]"], input[name="combo_price[]"]');
    ...
    function addComboTier() {
        var html = '<div class="combo-tier-row d-flex align-items-center mb-2">' +
            '<input type="number" name="combo_qty[]" placeholder="Min Qty" min="2" class="form-control mr-2" style="width:100px" required>' +
            '<input type="number" name="combo_price[]" placeholder="Price (৳)" min="0" step="0.01" class="form-control mr-2" required>' +
            '<button type="button" class="btn btn-icon btn-soft-danger btn-circle btn-sm" onclick="removeComboTier(this)">' +
            '<i class="las la-times"></i></button></div>';
        document.getElementById('combo_tiers').insertAdjacentHTML('beforeend', html);
    }
    function removeComboTier(btn) {
        var rows = document.querySelectorAll('.combo-tier-row');
        if (rows.length > 1) btn.closest('.combo-tier-row').remove();
    }
</script>
