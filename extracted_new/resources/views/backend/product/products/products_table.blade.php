    ...
    <table class="table mb-0" id="aiz-data-table">
         <thead>
            <tr>
                ...
                <th class="hide-xxl text-uppercase fs-12 fw-700 text-secondary text-danger"> {{ translate('Stock Out') }}</th>
                @endif
            ...
            <tr class="data-row">
                ...
                <td class="hide-md align-middle" data-label="Price Details">
                    <div class="border-width-3  border-left border-blue px-2 py-0 mb-1">
                        <span class="text-secondary fs-12 fw-400">{{ translate('Selling Price') }}</span>
                    ...
                    <div class="border-width-3  border-left border-warning px-2 py-0 mb-1">
                        <span class="text-secondary fs-12 fw-400">{{ translate('Buying Price') }}</span>
                        <p class="fs-14 fw-600 m-0">{{ single_price($product->purchase_price) }}</p>
                    </div>
                    @php $profit = $product->unit_price - $product->purchase_price; @endphp
                    <div class="border-width-3 border-left {{ $profit >= 0 ? 'border-success' : 'border-danger' }} px-2 py-0 mb-1">
                        <span class="text-secondary fs-12 fw-400">{{ translate('Profit') }}</span>
                        <p class="fs-14 fw-700 m-0 {{ $profit >= 0 ? 'text-success' : 'text-danger' }}">
                            {{ $profit >= 0 ? single_price($profit) : '-' . single_price(abs($profit)) }}
                            @if($product->purchase_price > 0)
                            <small class="fw-400">({{ number_format(($profit / $product->purchase_price) * 100, 1) }}%)</small>
                            @endif
                        </p>
                    </div>
                    @endif
                ...
                <td class="hide-xxl align-middle" data-label="StockOut">
                    @if (!$product->draft)
                    <label class="aiz-switch mb-0">
                        <input onchange="update_stock_out(this)" value="{{ $product->id }}"
                            type="checkbox" <?php if ($product->stock_out == 1) {
                                echo 'checked';
                            } ?>>
