...
if (!function_exists('cart_product_price')) {
    function cart_product_price($cart_product, $product, $formatted = true, $tax = true)
    {
        if ($product->auction_product == 0) {
            ...
            // Combo products: use the saved cart price directly (already calculated at add-to-cart time)
            if (preg_match('/^combo_\d+/', $str)) {
                $price = $cart_product['price'];
            } else {
                // Strip combo prefix (e.g. "combo_3_AliceBlue-Xl" → "AliceBlue-Xl")
                if (preg_match('/^combo_\d+_(.+)$/', $str, $m)) {
                    $str = $m[1];
                }
                $price = 0;
...
if (!function_exists('cart_product_tax')) {
    function cart_product_tax($cart_product, $product, $formatted = true)
    {
        ...
        // Combo products: use saved cart price for tax calculation
        if (preg_match('/^combo_\d+/', $str)) {
            $price = $cart_product['price'];
        } else {
            // Strip combo prefix
...
// Get single Color code (hex) from name
if (!function_exists('get_single_color_code')) {
    function get_single_color_code($colorName)
    {
        $color = Color::where('name', $colorName)->first();
        return $color ? $color->code : $colorName;
    }
}
