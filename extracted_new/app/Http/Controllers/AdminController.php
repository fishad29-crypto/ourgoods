...
class AdminController extends Controller
{
    ...
    public function admin_dashboard(Request $request)
    {
        ...
        // Profit & Finance Data
        $data['total_revenue'] = Order::where('delivery_status', 'delivered')->sum('grand_total');
        $data['total_products_sold'] = DB::table('order_details')
            ->join('orders', 'orders.id', '=', 'order_details.order_id')
            ->where('orders.delivery_status', 'delivered')
            ->sum('order_details.quantity');
        $data['total_cogs'] = DB::table('order_details')
            ->join('orders', 'orders.id', '=', 'order_details.order_id')
            ->join('products', 'products.id', '=', 'order_details.product_id')
            ->where('orders.delivery_status', 'delivered')
            ->select(DB::raw('COALESCE(SUM(order_details.quantity * products.purchase_price), 0) as total_cost'))
            ->first()->total_cost;
        $data['total_profit'] = $data['total_revenue'] - $data['total_cogs'];
        $data['profit_this_month'] = DB::table('order_details')
            ->join('orders', 'orders.id', '=', 'order_details.order_id')
            ->join('products', 'products.id', '=', 'order_details.product_id')
            ->where('orders.delivery_status', 'delivered')
            ->whereYear('orders.created_at', Carbon::now()->year)
            ->whereMonth('orders.created_at', Carbon::now()->month)
            ->select(DB::raw('COALESCE(SUM(order_details.price + order_details.tax), 0) as revenue, COALESCE(SUM(order_details.quantity * products.purchase_price), 0) as cost'))
            ->first();
        $data['revenue_this_month'] = $data['profit_this_month']->revenue ?? 0;
        $data['cost_this_month'] = $data['profit_this_month']->cost ?? 0;
        $data['profit_this_month'] = ($data['profit_this_month']->revenue ?? 0) - ($data['profit_this_month']->cost ?? 0);
