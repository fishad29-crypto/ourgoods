...
class HomeController extends Controller
{
    ...
    public function load_filtered_products_section(Request $request)
    {
        $filter = $request->filter ?? 'new_arrival';
        $limit = 24;
        $page = 1;
        ...
        switch ($filter) {
            case 'for_you':
                $newest_products = filter_products(Product::inRandomOrder())
                    ->skip($offset)
                    ->take($limit)
                    ->get();
                break;
            ...
            case 'best_seller':
                $newest_products = filter_products(Product::orderBy('num_of_sale', 'desc'))
                    ->skip($offset)
                    ->take($limit)
                    ->get();
                break;
            ...
            case 'combo_deals':
                $newest_products = filter_products(
                    Product::where('combo_deal', 1)
                        ->whereHas('comboDeals')
                        ->latest()
                )
                    ->skip($offset)
    ...
    public function variant_price(Request $request)
    {
        ...
        // Combo products allow mixing colors — use total stock across ALL variants
        if ($product->combo_deal && $product->comboDeals()->count() > 0) {
            $max_limit = $product->stocks->sum('qty');
        }
