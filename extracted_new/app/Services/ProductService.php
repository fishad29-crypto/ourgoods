...
class ProductService
{
    public function store(array $data)
    {
        ...
        $tags = array();
        if (isset($collection['tags']) && isset($collection['tags'][0]) && $collection['tags'][0] != null) {
            $decoded = json_decode($collection['tags'][0]);
            if (is_array($decoded)) {
        ...
        if ($collection['date_range'] != null) {
            ...
            $start = \DateTime::createFromFormat('d-m-Y H:i:s', trim($date_var[0]));
            $end   = \DateTime::createFromFormat('d-m-Y H:i:s', trim($date_var[1]));
            $discount_start_date = $start ? $start->getTimestamp() : strtotime($date_var[0]);
            $discount_end_date   = $end   ? $end->getTimestamp()   : strtotime($date_var[1]);
        }
        ...
        // Handle color images
        $color_images = null;
        if (isset($collection['color_images']) && is_array($collection['color_images'])) {
            $color_images = json_encode(array_filter($collection['color_images']));
        }
        unset($collection['color_images']);
    ...
    {
        $collection = collect($data);
        //Log::info('Product Update Data - date_range: ', ['date_range' => $data['date_range'] ?? 'NOT SET', 'discount' => $data['discount'] ?? 'NOT SET']);
        $slug = Str::slug($collection['name']);
        ...
        if(!isset($collection['stock_out'])){
            $collection['stock_out'] = 0;
        }
