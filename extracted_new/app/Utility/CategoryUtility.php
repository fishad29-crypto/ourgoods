...
class CategoryUtility
{
    ...
    {
        // Load ALL categories once to avoid N+1 recursive DB queries
        static $allCategories = null;
        if ($allCategories === null) {
            $allCategories = Category::orderBy('order_level', 'desc')->get()->toArray();
        }

        // Find children in memory instead of DB query per category
        $children = array_filter($allCategories, function($cat) use ($id) {
